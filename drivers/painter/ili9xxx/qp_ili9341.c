...
bool qp_ili9341_init(painter_device_t device, painter_rotation_t rotation) {
    ...
    qp_ili9xxx_command_databuf(device, ILI9XXX_POWER_CTL_A, power_ctl_a, sizeof(power_ctl_a));
    static const uint8_t power_ctl_b[] = {0x00, 0xD9, 0x30};
    qp_ili9xxx_command_databuf(device, ILI9XXX_POWER_CTL_B, power_ctl_b, sizeof(power_ctl_b));
    static const uint8_t power_on_seq[] = {0x64, 0x03, 0x12, 0x81};
    qp_ili9xxx_command_databuf(device, ILI9XXX_POWER_ON_SEQ_CTL, power_on_seq, sizeof(power_on_seq));
    qp_ili9xxx_command_databyte(device, ILI9XXX_SET_PUMP_RATIO_CTL, 0x20);
    qp_ili9xxx_command_databyte(device, ILI9XXX_SET_POWER_CTL_1, 0x26);
    qp_ili9xxx_command_databyte(device, ILI9XXX_SET_POWER_CTL_2, 0x11);
    ...
    qp_ili9xxx_command_databyte(device, ILI9XXX_SET_VCOM_CTL_2, 0xBE);

    // Configure timing control
    static const uint8_t drv_timing_ctl_a[] = {0x85, 0x10, 0x7A};
    qp_ili9xxx_command_databuf(device, ILI9XXX_DRV_TIMING_CTL_A, drv_timing_ctl_a, sizeof(drv_timing_ctl_a));
    static const uint8_t drv_timing_ctl_b[] = {0x00, 0x00};
    qp_ili9xxx_command_databuf(device, ILI9XXX_DRV_TIMING_CTL_B, drv_timing_ctl_b, sizeof(drv_timing_ctl_b));

    // Configure brightness / gamma
    qp_ili9xxx_command_databyte(device, ILI9XXX_SET_BRIGHTNESS, 0xFF);
    qp_ili9xxx_command_databyte(device, ILI9XXX_ENABLE_3_GAMMA, 0x00);
    qp_ili9xxx_command_databyte(device, ILI9XXX_SET_GAMMA, 0x01);
