...
static bool qgf_parse_frame_descriptor(qgf_frame_v1_t *frame_descriptor, uint8_t *bpp, bool *has_palette, bool *is_delta, painter_compression_t *compression_scheme, uint16_t *delay) {
    // clang-format off
    ...
    if (frame_descriptor->format > PALETTE_8BPP) {
        qp_dprintf("Failed to parse frame_descriptor, invalid format 0x%02X\n", (int)frame_descriptor->format);
        return false;
    } else {
        if (bpp) {
            *bpp = formats[frame_descriptor->format].bpp;
        }
        if (has_palette) {
            *has_palette = formats[frame_descriptor->format].has_palette;
        }
    }
...
static bool qgf_read_frame_offset(qp_stream_t *stream, uint16_t frame_number, uint32_t *frame_offset) {
    ...
    if (!qgf_read_graphics_descriptor(stream, NULL, NULL, &frame_count)) {
        return false;
    }
    ...
    uint32_t offset = 0;
    if (qp_stream_read(&offset, sizeof(uint32_t), 1, stream) != sizeof(uint32_t)) {
...
static void qgf_seek_to_frame_descriptor(qp_stream_t *stream, uint16_t frame_number) {
    // Read the offset
    uint32_t offset = 0;
...
bool qgf_prepare_frame_for_stream_read(qp_stream_t *stream, uint16_t frame_number, qgf_frame_info_t *info, qp_pixel_t *pixel_lookup_table) {
    ...
    if (info->has_palette) {
        ...
        uint16_t palette_entries = 1u << info->bpp;
        uint8_t  entry_length    = (sizeof(uint8_t) * 3);
        uint32_t expected_length = palette_entries * entry_length;

        // Don't bother updating the lookup table if no palette specified (validation)
        if (pixel_lookup_table) {
            // Read the palette entries
            for (uint16_t i = 0; i < palette_entries; ++i) {
                // Read the palette entry
                uint8_t entry[3];
                if (qp_stream_read(entry, sizeof(uint8_t), 3, stream) != entry_length) {
                    return false;
                }

                // Update the lookup table
                pixel_lookup_table[i].hsv888.h = entry[0];
                pixel_lookup_table[i].hsv888.s = entry[1];
                pixel_lookup_table[i].hsv888.v = entry[2];
            }
        } else {
            // Skip forward the amount of bytes in the palette
            qp_stream_seek(stream, expected_length, SEEK_CUR);
        }
    }
